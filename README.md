...
- **资源管理**：上传封面、添加图片或手动资源引用；大体积 `data:` 图片 URI 会折叠显示，并在导出卡片元数据时移除以提升 SillyTavern 导入兼容性，同时保留在编辑器草稿中供预览和再次保存使用。PNG 导出也会移除部分图片来源信息块，避免兼容性较差的导入端误判文件损坏。
...
7. 保存或导出 JSON、PNG/APNG、CHARX，用于 SillyTavern 或备份归档。PNG 导出会把封面作为图片本体，不会在卡片元数据里重复写入大体积内嵌图片，也会剥离可能影响导入兼容性的 PNG 来源信息块。
