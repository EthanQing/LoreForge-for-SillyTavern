...
Agent Studio 输入框的 `@` 候选由当前页面上下文限制。`AgentStudio.tsx` 将基础信息、提示词、开场白和世界书编辑页分别映射到对应 section 权限；卡片纲要页才使用整卡上下文，预览、资源、设置和项目文件页暂不提供 `@` 候选。右侧编辑台关闭后保留最后一个业务页面上下文，不能因为 `focusedEditor` 被清空而把世界书或开场白权限扩大为整张卡片。候选插入后，编辑器会将已完成的目标替换为隐藏 `@`/引号且可截断的不可编辑 token widget；光标移动不会进入 token 内部，Backspace/Delete 或覆盖 token 的选区会原子删除整个 token（必要时连同插入的分隔空格），但发送仍保留原始 token 文本。已发送的用户消息由 `MarkdownMessage` 使用相同的 mention 规则显示为静态 token 样式；`permissions.ts` 仍会按当前卡片重新解析目标，页面筛选不是唯一安全边界。
...
左侧工作区导航的“项目文件”负责打开、保存、导入和导出角色卡文件，“预览”用于查看当前卡片的只读展示效果；右侧“卡片纲要”提供 CCv3 字段编辑入口，并提供进入“预览”的快捷入口。“卡片资源”只管理当前卡片的 `card.data.assets`，提供主封面状态、用途说明、资源统计、摘要卡片和可折叠技术字段；它不是全局资源库。两者属于不同导航层级，新增入口时不要再使用不带范围说明的“资源”作为工作区文件入口。
