...
项目固定使用的 Pi AI 版本已实现 `openai-codex-responses` 模型协议与事件解析，并定义了对应设备码登录端点和客户端标识。Rust 后端已有系统凭据库与 AI HTTP/SSE 代理边界。

## 决策

- 使用 ChatGPT 设备码登录，不把它描述为通用 OpenAI API OAuth。
- Rust 执行设备码申请、轮询、token exchange、系统凭据存储与自动刷新；前端只显示 verification URL 和一次性 user code。
...
该能力依赖固定 Pi AI 版本所支持的 Codex 客户端协议；升级 Pi AI 时必须复核设备码端点、客户端标识、模型目录、Responses 路径和认证头。真实登录只能使用启用了设备码登录且具备相应 Codex 权限的 ChatGPT 账户验收。
