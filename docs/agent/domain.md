...
Agent 输入框输入 `@` 时会按标题、ID 和关键词列出当前卡片的世界书条目，并可滚动查看全部匹配项。方向键移动选项，`Tab` 确认，鼠标也可选择；可连续选择任意多个条目，确认后请求权限锁定到全部所选条目，聊天记录会保留原始 `@` 文本。带空格的标题使用引号编码，重名标题附带一基条目序号，由权限解析层再次校验后转换为具体条目权限。
