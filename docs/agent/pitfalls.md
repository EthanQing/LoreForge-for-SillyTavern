...
## SillyTavern 主世界书绑定不能只看内嵌名称

`data.character_book.name` 只是卡片内嵌世界书的名称；SillyTavern 还会从卡片 `data.extensions.world` 读取主世界书绑定，并据此查找外部世界书。导入、重命名或从旧卡片继续编辑时，两者可能暂时不同；Agent 只读投影应同时展示这两个值，校验路径使用 `data.extensions.world`，但不能把 extensions 变成普通 Agent 可写字段。

只有明确的内嵌世界书重命名 action 才能自动改写空绑定或仍等于旧内嵌名称的绑定；整本导入/替换不是重命名，必须保留现有绑定并提示用户显式同步。这样可以避免破坏用户有意共享的外部世界书。PNG/JSON 导出也不会替用户重命名 SillyTavern 数据库中的世界书。

SillyTavern 导入内嵌世界书时，会为缺失的条目 `id` 回退到数组索引，并以该值作为对象键。缺失 ID 如果与任何显式 ID 重合，后一个条目会覆盖前一个，表现为导入条目数减少。导出前必须先在前端和 Rust 两条路径中补齐字符串化后唯一的 UID，且不得因预览排序或手动排序重排底层数组。
