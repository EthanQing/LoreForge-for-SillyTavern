...
- `src/features/agent-studio/agentMention.test.ts` 覆盖 @ token 的格式、重复目标和候选范围；Agent 输入框手工检查时还应确认 Backspace/Delete 会原子删除完整 token。
...
- OpenAI Codex OAuth：Rust 单测覆盖 token claim、浏览器 PKCE 授权 URL、设备码响应、endpoint 限制和二进制凭据编码兼容；前端测试还要确认 Agent 绕过 Pi 的 OAuth 注册表，直接使用 Rust 凭据代理。真实登录需要在 ChatGPT 账户上手工验收，自动测试不得保存真实令牌。
