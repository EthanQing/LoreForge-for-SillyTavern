...
首个完整用户/Agent turn 可触发一次独立的无工具标题请求。标题模型只接收截断后的用户指令和 Agent 回复，并被要求输出 4–12 个汉字的简体中文短标题。前端移除引号、Markdown、前缀和尾部标点，并限制最终长度。

标题是 `agent_sessions.title` 元数据，不是 Agent 消息。标题请求不得复用主 Controller 的消息队列、权限状态或工具集合，也不得创建提案。请求或持久化失败时保留“新会话”，主会话继续正常工作。
