...
The middle transcript uses `src/lib/agent/transcript.ts` to group one user request and all assistant/tool-loop messages into one turn. The assistant text is rendered as one bubble, PI's `streamingMessage` is rendered in that same bubble while a run is active, and tool results are nested under a collapsed trace. The transcript owns vertical scrolling, constrains child widths, and wraps long JSON/path content so tool payloads cannot create horizontal overflow.
