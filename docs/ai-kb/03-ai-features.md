...
Edit mode keeps workflow actions behind the composer command menu instead of a persistent toolbar. Click the `+` control in the composer bottom toolbar or type `/` to select card diagnosis, draft completion, source extraction, consistency repair, token optimization, worldBook building, or import cleanup. The workflow menu closes when the user selects an item, clicks outside it, presses Escape, or toggles `+` again. Selecting a workflow shows a small workflow pill in the composer toolbar, keeps focus in the composer, and leaves the text area for optional user guidance instead of inserting the workflow's default instruction. Hovering the pill reveals an `x` control to cancel the workflow selection without deleting typed guidance. The actual workflow runs only when the user sends the message; the frontend combines the selected workflow's default instruction with any typed guidance internally. Workflows still return the same structured response and use preview/apply for any patches.
